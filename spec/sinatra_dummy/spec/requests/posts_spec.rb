...
# Written exactly as a Sinatra user would. rack-test is wired in by
# `require "openapi_ruby/rspec"`; only `app` is the suite's to name.
RSpec.describe "Posts API", type: :openapi do
  ...
  it "GET /api/v1/posts returns all posts" do
    PostStore.create(title: "First post", body: "Hello")
    PostStore.create(title: "Second post")
  ...
  it "GET /api/v1/posts filters by author" do
    PostStore.create(title: "Jane's post", author: "Jane")
    PostStore.create(title: "John's post", author: "John")
  ...
  it "GET /api/v1/posts/:id returns a post" do
    post = PostStore.create(title: "My post", body: "Content")
  ...
  it "DELETE /api/v1/posts/:id deletes a post" do
    post = PostStore.create(title: "Delete me")

    assert_api_response :delete, 204, path_params: {id: post[:id]}

    expect(PostStore.find(post[:id])).to be_nil
  end
